...
class NurseryPlantCareController extends Controller
{
    ...
    {
        $query = NurseryPlantCareRecord::where('user_id', $request->user()->id)
            ->with('plant');

        if ($request->filled('plant_id')) {
            $query->where('nursery_plant_id', $request->plant_id);
        }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $record = NurseryPlantCareRecord::create([
            ...$validated,
