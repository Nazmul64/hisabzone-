...
class NurseryDeliveryController extends Controller
{
    ...
    {
        $query = NurseryDelivery::where('user_id', $request->user()->id)->with(['order', 'customer']);
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $last = NurseryDelivery::where('user_id', $request->user()->id)->orderByDesc('id')->first();
        ...
        return response()->json(['success' => true, 'data' => $delivery->load(['order', 'customer']), 'message' => 'Delivery created successfully'], 201);
    }

    public function show(Request $request, $id): JsonResponse
    {
        $delivery = NurseryDelivery::where('user_id', $request->user()->id)
            ->with(['order', 'customer'])->findOrFail($id);
    ...
    {
        $delivery  = NurseryDelivery::where('user_id', $request->user()->id)->findOrFail($id);
        $validated = $request->validate([
            'nursery_order_id'    => 'nullable|integer|exists:nursery_orders,id',
            'nursery_customer_id' => 'nullable|integer|exists:nursery_customers,id',
            'customer_name'       => 'nullable|string|max:255',
        ...
        return response()->json(['success' => true, 'data' => $delivery->fresh(['order', 'customer']), 'message' => 'Delivery updated successfully']);
    }

    public function destroy(Request $request, $id): JsonResponse
    {
        NurseryDelivery::where('user_id', $request->user()->id)->findOrFail($id)->delete();

        return response()->json(['success' => true, 'message' => 'Delivery deleted successfully']);
    }
}
