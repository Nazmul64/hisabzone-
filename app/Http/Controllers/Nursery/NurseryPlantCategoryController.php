...
class NurseryPlantCategoryController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        $categories = NurseryPlantCategory::where('user_id', $request->user()->id)
            ->withCount('plants')->orderBy('name')->get();
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $category = NurseryPlantCategory::create([...$validated, 'user_id' => $request->user()->id]);
    ...
    {
        $category = NurseryPlantCategory::where('user_id', $request->user()->id)->findOrFail($id);
    ...
    {
        NurseryPlantCategory::where('user_id', $request->user()->id)->findOrFail($id)->delete();
