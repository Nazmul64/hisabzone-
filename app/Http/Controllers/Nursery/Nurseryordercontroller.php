...
class NurseryOrderController extends Controller
{
    ...
    {
        $query = NurseryOrder::where('user_id', $request->user()->id)->with(['customer', 'plant']);
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $last = NurseryOrder::where('user_id', $request->user()->id)->orderByDesc('id')->first();
        $num  = $last ? (intval(substr($last->order_number ?? '#ORD-000', 5)) + 1) : 1;
        $validated['order_number'] = '#ORD-' . str_pad($num, 3, '0', STR_PAD_LEFT);
        $validated['user_id']      = $request->user()->id;

        $order = NurseryOrder::create($validated);

        return response()->json(['success' => true, 'data' => $order->load(['customer', 'plant']), 'message' => 'Order created successfully'], 201);
    }
    ...
    {
        $order     = NurseryOrder::where('user_id', $request->user()->id)->findOrFail($id);
        $validated = $request->validate([
            'nursery_customer_id' => 'nullable|integer|exists:nursery_customers,id',
            'nursery_plant_id'    => 'nullable|integer|exists:nursery_plants,id',
            'customer_name'       => 'nullable|string|max:255',
        ...
        return response()->json(['success' => true, 'data' => $order->fresh(['customer', 'plant']), 'message' => 'Order updated successfully']);
    }

    public function destroy(Request $request, $id): JsonResponse
    {
        NurseryOrder::where('user_id', $request->user()->id)->findOrFail($id)->delete();
